...
$db_host = "localhost";
$db_user = "root";
$db_pass = ""; // Empty password for local XAMPP default setup
$db_name = "jewellery_db"; // Changed to a more specific name
...
try {
    ...
    if (!mysqli_select_db($con, $db_name)) {
        ...
        if (mysqli_query($con, $createDbQuery)) {
            ...
            $createTableQuery = "CREATE TABLE IF NOT EXISTS usersignup (
            ...
            throw new Exception("Error creating database: " . mysqli_error($con));
        }
    }
    
    // Now we have a valid connection with the database selected
} catch (Exception $e) {
    $message = "<p class='text-red-600 font-semibold mb-4'>Database Error: " . htmlspecialchars($e->getMessage()) . "</p>";
}

// Process form only if database connection is established
if (isset($con) && $con && empty($message) && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $checkEmailStmt = $con->prepare("SELECT id FROM usersignup WHERE email = ?");
        $checkEmailStmt->bind_param("s", $email_db);
        $checkEmailStmt->execute();
        $checkEmailStmt->store_result();
        
        if ($checkEmailStmt->num_rows > 0) {
            $errors['email'] = "This email is already registered.";
            $message = "<ul class='text-red-600 font-semibold mb-4 list-disc list-inside'><li>This email is already registered.</li></ul>";
        } else {
    ...
    // Only close connection if we're not redirecting
    if (!headers_sent()) {
        mysqli_close($con);
    }
}
