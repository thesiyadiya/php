...
$db_name = "project"; // Changed to a more specific name
...
if (isset($con) && $con && empty($message) && $_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email)) {
        ...
        $checkStmt = $con->prepare("SELECT email FROM usersignup WHERE email = ?");
    ...
    if (empty($errors)) {
        ...
        $message = "<div class='bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-6'>";
        $message .= "<h4 class='font-bold'>Registration Error</h4>";
